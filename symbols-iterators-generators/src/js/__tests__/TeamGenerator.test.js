...
describe('TeamGenerator with generator', () => {
  ...
  test('should verify generator creates new iterator on each call', () => {
    const team = new TeamGenerator();
    const char1 = { name: 'Лучник', type: 'Bowman', health: 50, level: 1, attack: 40, defence: 10 };
    const char2 = { name: 'Маг', type: 'Magician', health: 30, level: 1, attack: 50, defence: 5 };
    ...
    const iterator1 = team[Symbol.iterator]();
    const iterator2 = team[Symbol.iterator]();

    expect(iterator1).not.toBe(iterator2);
    expect(iterator1.next().value).toBe(char1);
    expect(iterator2.next().value).toBe(char1);
  });
});
